    ...
    return (
        <div className="animate-fade">
            <header className="content-header">
                <div>
                    <h2>전술 요원 통합 관리 🛡️</h2>
                    <p>요원들의 가입을 승인하고 적절한 작전 권한을 부여합니다.</p>
                </div>
            </header>

            <section className="admin-section glass-card">
                <div className="section-header">
                    <h3>신규 요원 승인 대기 ({users.filter(u => !u.isApproved).length})</h3>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                                ...
                                <tr key={u.id}>
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                                            <button className="btn-icon approve" onClick={() => onApproveUser(u.id, 'viewer')} title="열람요원 승인"><Eye size={16} /> {roleNames.viewer} 승인</button>
                                            <button className="btn-icon approve" style={{ borderColor: '#818cf8', color: '#818cf8' }} onClick={() => onApproveUser(u.id, 'editor')} title="실행요원 승인"><Edit3 size={16} /> {roleNames.editor} 승인</button>
                            ...
                            {users.filter(u => !u.isApproved).length === 0 && (
                                <tr><td colSpan="4" style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>현재 검증 대기 중인 요원이 없습니다.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            <section className="admin-section glass-card" style={{ marginTop: '2rem' }}>
                ...
                <div className="table-wrapper">
                    <table>
                        <thead>
                            <tr><th>이름</th><th>아이디</th><th>현재 보직</th><th>상태</th><th>액션</th></tr>
                        </thead>
                        <tbody>
                            {users.filter(u => u.isApproved).map(u => (
                                <tr key={u.id}>
                                    ...
                                    <td><span className="status-badge success">작전 중</span></td>
                                    <td>
            ...
            {/* 요원 정보 수정 모달 (Admin 전용) */}
            {editingUser && (
                <div className="modal-overlay">
                    <div className="modal-content glass" style={{ width: '400px' }}>
                        <div className="modal-header"><h3>요원 강제 옵션 수정</h3><button onClick={() => setEditingUser(null)} style={{ background: 'none', border: 'none', color: '#94a3b8' }}><X /></button></div>
                        <div className="input-group"><label>요원명</label><input type="text" id="edit-user-name" className="input-field" defaultValue={editingUser.name} /></div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
                            ...
                            <button className="btn btn-primary" onClick={async () => {
                                ...
                                try {
                                    ...
                                    setNotification({ type: 'success', message: `${editingUser.name} 요원 정보가 클라우드에 업데이트되었습니다.` });
                                    setEditingUser(null);
                                } catch (err) {
